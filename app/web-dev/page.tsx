  ...
  return (
    <div className="relative h-px overflow-hidden" style={{ background: 'linear-gradient(90deg, transparent 0%, rgba(255,255,255,0.3) 30%, rgba(255,255,255,0.3) 70%, transparent 100%)' }}>
      <div className="absolute inset-0 animate-[sweep_4s_ease-in-out_infinite]"
        style={{ background: 'linear-gradient(90deg, transparent, rgba(255,255,255,0.7), transparent)', width: '40%', animation: 'rule-sweep 4s ease-in-out infinite' }}
      />
    </div>
  )
}
...
export default function WebDevPage() {
  ...
  const [visible, setVisible] = useState<Set<Element>>(new Set())
  ...
  const isVisible = (el: Element | null) => el ? visible.has(el) : false
  ...
  return (
    <>
      ...
      <style>{`
        ...
        .wd-rv { opacity:0; transform:translateY(28px); transition: opacity .9s cubic-bezier(.16,1,.3,1), transform .9s cubic-bezier(.16,1,.3,1); }
        .wd-rv.in { opacity:1; transform:none; }
        .wd-rv.d1 { transition-delay:.1s }
        .wd-rv.d2 { transition-delay:.22s }
        .wd-rv.d3 { transition-delay:.36s }
        .eco-card { background:#111; border:1px solid rgba(255,255,255,.06); transition:all .45s cubic-bezier(.16,1,.3,1); }
        ...
        .cap-card { flex-shrink:0; width:340px; background:#111; border:1px solid rgba(255,255,255,.06); padding:52px 44px; position:relative; overflow:hidden; transition:all .4s cubic-bezier(.16,1,.3,1); }
        ...
        .proc-step { background:#111; border:1px solid rgba(255,255,255,.06); padding:56px 52px; position:relative; overflow:hidden; transition:background .4s; }
        .proc-step:hover { background:#1a1a1a; }
        .why-row { padding:28px 0; border-bottom:1px solid rgba(255,255,255,.06); display:flex; gap:24px; align-items:flex-start; transition:all .35s; }
        .why-row:last-child { border-bottom:none; }
        .why-row:hover .why-num { color:#ffffff !important; }
        .ed-cell:hover img { transform:scale(1.06); }
        .btn-primary { background:#ffffff; color:#000000; font-size:.6rem; font-weight:700; letter-spacing:.2em; text-transform:uppercase; padding:16px 40px; display:inline-block; position:relative; overflow:hidden; transition:all .35s ease; }
        .btn-primary::before { content:''; position:absolute; inset:0; background:rgba(0,0,0,.1); transform:skewX(-15deg) translateX(-120%); transition:transform .5s; }
        .btn-primary:hover { background:#e5e5e5; transform:translateY(-2px); box-shadow:0 16px 40px rgba(255,255,255,.15); }
        .btn-primary:hover::before { transform:skewX(-15deg) translateX(220%); }
        .btn-outline { border:1px solid rgba(255,255,255,.3); color:#ffffff; font-size:.6rem; font-weight:500; letter-spacing:.2em; text-transform:uppercase; padding:16px 40px; display:inline-block; transition:all .35s; }
        .btn-outline:hover { border-color:#ffffff; background:rgba(255,255,255,.08); }
      `}</style>

      {/* Cursor */}
      <div id="wd-dot" className="fixed pointer-events-none z-[9999]" style={{ mixBlendMode:'difference' }}>
        <div style={{ width:6, height:6, borderRadius:'50%', background:'#ffffff', transform:'translate(-50%,-50%)' }}/>
      </div>
      <div id="wd-ring" className="fixed pointer-events-none z-[9998]">
        <div style={{ width:38, height:38, borderRadius:'50%', border:'1px solid rgba(255,255,255,.4)', transform:'translate(-50%,-50%)', transition:'width .3s, height .3s' }}/>
      </div>
      ...
      <main style={{ backgroundColor:'#000000', color:'#ffffff', fontFamily:'var(--font-montserrat)' }}>
        ...
        <section style={{ height:'100vh', minHeight:700, display:'grid', gridTemplateColumns:'1fr 1fr', overflow:'hidden', position:'relative' }}>
          ...
          <div style={{ display:'flex', flexDirection:'column', justifyContent:'flex-end', padding:'0 3.75rem 90px', position:'relative', zIndex:3, background:'linear-gradient(110deg, #000 40%, rgba(0,0,0,.7) 100%)' }}>
            <p className="font-body" style={{ fontFamily:'var(--font-montserrat)', fontSize:'.58rem', letterSpacing:'.35em', textTransform:'uppercase', color:'rgba(255,255,255,0.6)', marginBottom:28, animation:'wd-up .8s .3s ease forwards', opacity:0 }}>
            ...
            <div style={{ display:'flex', gap:16, flexWrap:'wrap', animation:'wd-up .9s .9s ease forwards', opacity:0 }}>
          ...
          <div style={{ position:'relative', overflow:'hidden' }}>
            ...
            {/* Floating badge */}
            <div style={{ position:'absolute', bottom:60, right:20, zIndex:4, background:'rgba(0,0,0,.82)', backdropFilter:'blur(12px)', border:'1px solid rgba(255,255,255,.15)', padding:'24px 32px', width:220, animation:'wd-up .9s 1.1s ease forwards', opacity:0 }}>
              <div className="font-heading" style={{ fontFamily:'var(--font-cormorant)', fontSize:'2.6rem', lineHeight:1, color:'#ffffff', marginBottom:8 }}>3.4×</div>
        ...
        <RevealSection style={{ padding:'6rem 3.75rem', position:'relative', overflow:'hidden', background:'#0a0a0a' }}>
          <div style={{ maxWidth:1280, margin:'0 auto', display:'grid', gridTemplateColumns:'1fr 1.1fr', gap:100, alignItems:'center' }}>
            <div>
              <div className="font-heading" style={{ fontFamily:'var(--font-cormorant)', fontSize:'8rem', lineHeight:.6, color:'rgba(255,255,255,.06)', marginBottom:-16, display:'block' }}>&ldquo;</div>
              <blockquote className="font-heading" style={{ fontFamily:'var(--font-cormorant)', fontSize:'clamp(2.4rem,3.8vw,4rem)', fontWeight:400, lineHeight:1.18, color:'#ffffff' }}>
        ...
        <RevealSection style={{ padding:'6rem 3.75rem', background:'#000000' }}>
          <div style={{ maxWidth:1280, margin:'0 auto 80px', display:'flex', justifyContent:'space-between', alignItems:'flex-end' }}>
            <div>
              ...
              <h2 className="font-heading" style={{ fontFamily:'var(--font-cormorant)', fontSize:'clamp(2rem,3.2vw,3rem)', fontWeight:400, lineHeight:1.2, color:'#ffffff' }}>
                A Complete <em style={{ fontStyle:'italic', color:'rgba(255,255,255,.7)' }}>Digital Ecosystem</em>
              </h2>
            </div>
            <p style={{ fontSize:'.78rem', lineHeight:2, color:'rgba(255,255,255,.5)', maxWidth:360, textAlign:'right' }}>Six interconnected disciplines. One unified architecture. Engineered to perform at the highest level of digital commerce and brand communication.</p>
          </div>

          <div style={{ maxWidth:1280, margin:'0 auto', display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:3 }}>
            {/* Image card — spans 2 rows */}
            <div className="eco-card" style={{ gridColumn:'span 1', gridRow:'span 2', minHeight:420, padding:0, overflow:'hidden', position:'relative' }}>
              ...
              <div style={{ position:'absolute', bottom:32, left:36, fontFamily:'var(--font-montserrat)', fontSize:'.58rem', letterSpacing:'.3em', textTransform:'uppercase', color:'rgba(255,255,255,.6)' }}>Digital Architecture</div>
            </div>
            {/* Cards 01–06 */}
            {[
            ...
            <div ref={trackRef} style={{ display:'flex', gap:3, transition:'transform .65s cubic-bezier(.16,1,.3,1)', willChange:'transform' }}>
              {capabilities.map((cap, i) => (
                <div key={i} className="cap-card">
                  ...
                  <div className="font-heading" style={{ fontFamily:'var(--font-cormorant)', fontSize:'1.6rem', fontWeight:400, color:'#ffffff', marginBottom:20, lineHeight:1.2 }}>{cap.title}</div>
        ...
        <RevealSection style={{ padding:'6rem 3.75rem', background:'#000000', position:'relative', overflow:'hidden' }}>
          ...
          <div style={{ position:'relative', zIndex:2, maxWidth:1280, margin:'0 auto' }}>
            ...
            <h2 className="font-heading" style={{ fontFamily:'var(--font-cormorant)', fontSize:'clamp(2rem,3.2vw,3rem)', fontWeight:400, color:'#ffffff', marginBottom:80 }}>
        ...
        <RevealSection style={{ padding:'6rem 3.75rem', background:'#0a0a0a' }}>
          <div style={{ maxWidth:1280, margin:'0 auto', display:'grid', gridTemplateColumns:'1fr 1fr', gap:3 }}>
              ...
              <div style={{ position:'absolute', bottom:48, left:40, right:40, display:'flex', gap:2 }}>
                {[['98%','Client retention'],['40+','Countries served'],['3.4×','Avg. conversion uplift']].map(([n,l],i) => (
                  <div key={i} style={{ flex:1, background:'rgba(0,0,0,.8)', backdropFilter:'blur(12px)', border:'1px solid rgba(255,255,255,.12)', padding:'24px 20px', textAlign:'center' }}>
                    <div className="font-heading" style={{ fontFamily:'var(--font-cormorant)', fontSize:'2.2rem', color:'#ffffff', lineHeight:1, marginBottom:6 }}>{n}</div>
            ...
            <div style={{ background:'#111', border:'1px solid rgba(255,255,255,.06)', padding:'72px 64px', display:'flex', flexDirection:'column', justifyContent:'center' }}>
              ...
              <h2 className="font-heading" style={{ fontFamily:'var(--font-cormorant)', fontSize:'clamp(2rem,2.8vw,2.8rem)', fontWeight:400, lineHeight:1.2, color:'#ffffff', marginBottom:20 }}>
                Why International<br />Brands Choose <em style={{ fontStyle:'italic', color:'rgba(255,255,255,.7)' }}>Enhanccee®</em>
              </h2>
              <p style={{ fontSize:'.8rem', lineHeight:2, color:'rgba(255,255,255,.5)', marginBottom:52 }}>We are not an agency. We are a digital atelier — a focused team of specialists who hold themselves to one standard: exceptional.</p>
              <div>
                {whyRows.map((row, i) => (
                  <div key={i} className="why-row">
                    <div className="why-num font-heading" style={{ fontFamily:'var(--font-cormorant)', fontSize:'1.4rem', color:'rgba(255,255,255,.2)', flexShrink:0, lineHeight:1.3, minWidth:32, transition:'color .35s' }}>0{i+1}</div>
                    <div>
        ...
        <RevealSection style={{ position:'relative', overflow:'hidden', height:580, display:'flex', alignItems:'center' }}>
          ...
          <div style={{ position:'relative', zIndex:2, width:'100%', textAlign:'center', padding:'0 60px' }}>
            ...
            <h2 className="font-heading" style={{ fontFamily:'var(--font-cormorant)', fontSize:'clamp(2.6rem,5.5vw,5.5rem)', fontWeight:400, lineHeight:1.1, color:'#ffffff', maxWidth:1280, margin:'0 auto 32px' }}>
        ...
        {/* ══════════════ 8 — FINAL CTA ══════════════ */}
        <RevealSection style={{ padding:'6rem 3.75rem', background:'#000000', position:'relative', textAlign:'center', overflow:'hidden' }}>
          ...
          <div style={{ position:'relative', zIndex:2, maxWidth:1280, margin:'0 auto' }}>
            ...
            <h2 className="font-heading" style={{ fontFamily:'var(--font-cormorant)', fontSize:'clamp(2.6rem,4.5vw,4.4rem)', fontWeight:400, lineHeight:1.12, color:'#ffffff', marginBottom:24 }}>
              Your digital<br /><em style={{ fontStyle:'italic', color:'rgba(255,255,255,.7)' }}>architecture awaits.</em>
            </h2>
            <p style={{ fontSize:'.8rem', lineHeight:2, color:'rgba(255,255,255,.5)', marginBottom:64 }}>Select clients only. We partner with a limited number of brands each quarter — ensuring every project receives the full weight of our attention and expertise.</p>
